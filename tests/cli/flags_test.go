...
func TestParseFlags_AllOptions(t *testing.T) {
	...
	os.Args = []string{"mailgrid", "--env", "cfg.json", "--csv", "rec.csv", "--template", "body.html", "--subject", "Hello", "--dry-run", "--preview", "--port", "9090", "--concurrency", "3", "--retries", "4", "--batch-size", "2", "--filter", "tier = \"pro\""}
