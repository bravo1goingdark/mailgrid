...
func TestAddJob(t *testing.T) {
	...
	job, err := scheduler.NewJob(args, time.Time{}, "", "1s")
	assert.NoError(t, err)
...
func TestListJobs(t *testing.T) {
	...
	job1, err := scheduler.NewJob(args1, time.Time{}, "", "1s")
	assert.NoError(t, err)
	err = sched.AddJob(job1, func(j types.Job) error { return nil })
	assert.NoError(t, err)

	args2 := types.CLIArgs{Subject: "Job 2"}
	job2, err := scheduler.NewJob(args2, time.Time{}, "", "1s")
	assert.NoError(t, err)
...
func TestJobExecution(t *testing.T) {
	...
	job, err := scheduler.NewJob(args, time.Now().Add(100*time.Millisecond), "", "")
	assert.NoError(t, err)
